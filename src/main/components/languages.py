"""languages 组件：commit 加权语言统计（devicon 图标 + 可见百分比）。

数据来自 data/languages.json（数据源层 issue #2 产出）：
``[{"lang": ..., "weight": ..., "pct": ...}, ...]``，已剔除 vendor、
含组织/外部仓库贡献；组件只负责展示前 top 项。
"""
...
DEFAULT_HEADER = "🧑‍💻 语言（按 commit 加权，含组织/外部贡献）"
DEFAULT_TOP = 8
...
def render(config: dict[str, Any], data: dict[str, Any]) -> str:
    ...
    for item in items[:top]:
        ...
        label = f"<sub><b>{lang}</b> {pct:.1f}%</sub>"
        units.append(f"{icon} {label}".strip())
    if not units:
