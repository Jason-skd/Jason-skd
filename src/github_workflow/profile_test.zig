...
const FakeTransport = struct {
    ...
    fn send(context: *anyopaque, allocator: std.mem.Allocator, request: Request) anyerror!RawResponse {
        ...
        if (std.mem.find(u8, payload, "\"maxRepositories\":2") != null) self.saw_limit = true;
        if (std.mem.find(u8, payload, "\"after\":\"cursor-1\"") != null) self.saw_cursor_one = true;
        if (std.mem.find(u8, payload, "\"after\":\"cursor-2\"") != null) self.saw_cursor_two = true;
        if (std.mem.find(u8, payload, "2000-02-29T00:00:00Z") != null) self.saw_leap_day = true;
        if (std.mem.find(u8, payload, "9999-12-31T23:59:59Z") != null) self.saw_maximum = true;
